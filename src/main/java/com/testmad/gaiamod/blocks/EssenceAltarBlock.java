...
public class EssenceAltarBlock extends BlockContainer {
	...
	public void registerBlockIcons(IIconRegister iconRegister) {
		...
		this.essenceAltarTop = iconRegister.registerIcon(References.MODIDLOCAL
				+ ":"
		...
		// + getUnlocalizedName().substring(5) + "_top");
		this.essenceAltarFront = iconRegister
				.registerIcon(References.MODIDLOCAL
						+ ":"
						+ (this.isPowered ? getUnlocalizedName().substring(5)
								+ "_front"
								: (this.isActive ? getUnlocalizedName()
										.substring(5) + "_front"
	...
	public IIcon getIcon(int side, int metadata) {
		...
		return side == 1 || side == 0 ? this.essenceAltarTop : (metadata == 0
				&& side == 3 ? this.essenceAltarFront
				: (side != metadata ? this.blockIcon : this.essenceAltarFront));
	}
	...
	public static void updateBlockState(boolean isAltaring, boolean powered,
		...
		if (isAltaring && powered) {
			world.setBlock(xCoord, yCoord, zCoord,
					ModBlocks.essenceAltarBlockActive);
		} else if (!isAltaring && powered) {
			world.setBlock(xCoord, yCoord, zCoord,
					ModBlocks.essenceAltarBlockIdlePower);
		} else {
			world.setBlock(xCoord, yCoord, zCoord,
					ModBlocks.essenceAltarBlockIdle);
		}
		keepInventory = false;
	...
	public void breakBlock(World world, int x, int y, int z, Block oldblock,
			int oldMetadata) {
		if (!keepInventory) {
			TileEntityEssenceAltar tileentity = (TileEntityEssenceAltar) world
					.getTileEntity(x, y, z);
